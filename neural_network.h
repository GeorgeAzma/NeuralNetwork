...
class NeuralNetwork
{
  public:
    ...
    {
        return (size_t(std::max_element(getOutput().begin(), getOutput().end()) - getOutput().begin())) == size_t(std::max_element(labels.begin(), labels.end()) - labels.begin());
    }
    uint32_t getInputCount() const
    {
        return topology.front();
    }
    ...
    uint32_t getLayerSize(size_t layer) const
    {
        return topology[layer];
    }
    float getWeight(size_t layer, size_t neuron, size_t weight) const
    {
        return weights[layer][neuron][weight];
    }
    const std::vector<std::vector<std::vector<float>>> &getWeights() const
    {
        return weights;
    }
    float getBias(size_t layer, size_t neuron) const
    {
        return biases[layer][neuron];
    }
    const std::vector<std::vector<float>> &getBiases() const
    {
        return biases;
    }
    float calculateLoss(const std::vector<float> &labels)
    {
        return (*cost_function)(labels, getOutput());
    }
    const std::vector<float> &getOutput() const
    {
        return activated_neurons.back();
    }

    template <typename T, typename... Args>
    void setCostFunction(Args... args)
    {
        cost_function = std::make_shared<T>(std::forward<Args>(args)...);
    }
    template <typename T, typename... Args>
    void setOptimizer(Args... args)
    {
        optimizer = std::make_shared<T>(std::forward<Args>(args)...);
    }
    void setWeight(float value, uint32_t layer, uint32_t neuron, uint32_t weight)
    {
        weights[layer][neuron][weight] = value;
    }
    void setBias(float value, uint32_t layer, uint32_t neuron)
    {
